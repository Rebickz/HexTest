...
-- `address`
SELECT ST.first_name AS 'FIRST NAME', ST.last_name 'LAST NAME', AD.address 'ADDRESS' FROM staff ST INNER JOIN address AD ON ST.address_id = AD.address_id;
select * from staff;
...
SELECT CU.first_name, CU.last_name, CU.email, CO.country FROM 
customer CU INNER JOIN  address AD ON CU.address_id = AD.address_id
INNER JOIN city CT ON AD.city_id = CT.city_id 
INNER JOIN country CO ON CT.country_id = CO.country_id WHERE CO.country LIKE 'Canada';
